...
Телеграм-бот для поиска товаров на [wildberries.ru](https://www.wildberries.ru/) по названию и минимальной цене.
...
1. Используйте `/search` для поиска товаров: бот спросит название и минимальную цену.
2. Управляйте фильтром запрещённых слов: `/banned` — показать, `/ban_add` — добавить, `/ban_remove` — удалить.
3. При необходимости вернитесь к платёжной модели, включив команды подписки в `bot/main.py` (см. комментарии в коде).
