...
- Оплата подписки через YooKassa с проверкой статуса внутри Telegram.
...
YOOKASSA_RETURN_URL=https://example.com/success
```
...
5. Для платных тарифов бот сформирует ссылку YooKassa и кнопку "Проверить оплату". После успешной оплаты подписка активируется автоматически.
